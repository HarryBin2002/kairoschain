...
for dir in $proto_dirs; do
  ...
  for file in $proto_files; do
    # Check if the go_package in the file is pointing to kairoschain
    if grep -q "option go_package.*kairoschain" "$file"; then
...
cp -r github.com/HarryBin2002/kairoschain/v*/* ./
rm -rf github.com
