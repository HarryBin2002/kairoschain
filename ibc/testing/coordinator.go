...
// NewCoordinator initializes Coordinator with N EVM TestChain's (Kairoschain apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if kairoschainChain, ok := chain.App.(*app.Kairoschain); ok {
		bondDenom = kairoschainChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
