...
import (
	...
	evm "github.com/HarryBin2002/kairoschain/v12/x/evm/types"
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	suite.KairoschainChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(1))
	suite.IBCOsmosisChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(2))
	suite.IBCCosmosChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(3))
	suite.coordinator.CommitNBlocks(suite.KairoschainChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCOsmosisChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCCosmosChain, 2)

	s.app = suite.KairoschainChain.App.(*app.Kairoschain)
	evmParams := s.app.EvmKeeper.GetParams(s.KairoschainChain.GetContext())
	evmParams.EvmDenom = constants.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.KairoschainChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.KairoschainChain.GetContext(), big.NewInt(1))

	// Set block proposer once, so its carried over on the ibc-go-testing suite
	validators := s.app.StakingKeeper.GetValidators(suite.KairoschainChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.KairoschainChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.KairoschainChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.KairoschainChain.GetContext(), sdk.ConsAddress(suite.KairoschainChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.KairoschainChain.GetContext(), minttypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.KairoschainChain.GetContext(), minttypes.ModuleName, suite.KairoschainChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.KairoschainChain.GetContext(), types.ModuleName, coins)
	...
	err = s.app.Erc20Keeper.SetParams(suite.KairoschainChain.GetContext(), params)
	suite.Require().NoError(err)

	suite.pathOsmosisKairoschain = ibctesting.NewTransferPath(suite.IBCOsmosisChain, suite.KairoschainChain) // clientID, connectionID, channelID empty
	suite.pathCosmosKairoschain = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.KairoschainChain)
	suite.pathOsmosisCosmos = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.IBCOsmosisChain)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisKairoschain) // clientID, connectionID, channelID filled
	ibctesting.SetupPath(suite.coordinator, suite.pathCosmosKairoschain)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisCosmos)
	suite.Require().Equal("07-tendermint-0", suite.pathOsmosisKairoschain.EndpointA.ClientID)
	suite.Require().Equal("connection-0", suite.pathOsmosisKairoschain.EndpointA.ConnectionID)
	suite.Require().Equal("channel-0", suite.pathOsmosisKairoschain.EndpointA.ChannelID)

	nativeCoin = sdk.NewCoin(constants.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(nativeCoin)
	err = s.app.BankKeeper.MintCoins(suite.KairoschainChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.KairoschainChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Kairoschain chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.KairoschainChain.GetContext(),
		s.KairoschainChain.App.(*app.Kairoschain),
		suite.KairoschainChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/ukai") as input parameter.
