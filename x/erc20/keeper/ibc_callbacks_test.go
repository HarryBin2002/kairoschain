...
import (
	...
	"github.com/HarryBin2002/kairoschain/v12/x/erc20/keeper"
	...
	vestingtypes "github.com/HarryBin2002/kairoschain/v12/x/vesting/types"
)
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	secpAddrKairoschain := secpAddr.String()
	...
	ethsecpAddrKairoschain := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> Kairoschain IBC relayer
	sourceChannel := "channel-292"
	kairoschainChannel := "channel-3"
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, kairoschainChannel)
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrKairoschain, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:     secpAddr,
			disableERC20: true,
			ackSuccess:   true,
		...
		},
		{
			...
			malleate: func() {
				...
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   secpAddr,
			ackSuccess: false,
			expErc20s:  big.NewInt(0),
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrKairoschain, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625", "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "100", secpAddrCosmos, ethsecpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			ackSuccess: true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(erc20Denom, "100", secpAddrCosmos, ethsecpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			ackSuccess: true,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "500", otherSecpNativeAddr, ethsecpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			ackSuccess:       true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", secpAddrCosmos, secpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   secpAddr,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData("b/d//s/ss/", "100", ethsecpAddrKairoschain, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   secpAddr,
		...
		},
		{
			...
			malleate: func() {
				sourcePrefix := transfertypes.GetDenomPrefix(transfertypes.PortID, sourceChannel)
				prefixedDenom := sourcePrefix + registeredDenom
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "100", secpAddrCosmos, secpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "1000", secpAddrCosmos, ethsecpAddrKairoschain, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, kairoschainChannel, timeoutHeight, 0)
			},
			receiver:   ethsecpAddr,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, kairoschainChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, kairoschainChannel, 1)
...
func (suite *KeeperTestSuite) TestConvertCoinToERC20FromPacket() {
	senderAddr := "kairos1x2w87cvt5mqjncav4lxy8yfreynn273xx7q2u6"
