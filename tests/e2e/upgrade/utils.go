...
// KairoschainVersion is a custom comparator for sorting semver version strings.
type KairoschainVersion []string

// Len is the number of stored versions..
func (v KairoschainVersion) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v KairoschainVersion) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v KairoschainVersion) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := KairoschainVersion([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(KairoschainVersion(versions))
