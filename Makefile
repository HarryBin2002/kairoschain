...
HTTPS_GIT := https://github.com/HarryBin2002/kairoschain.git
DOCKER := $(shell which docker)
NAMESPACE := kairoschain
PROJECT := kairoschain
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=kairoschain \
...
build-docker:
	...
	$(DOCKER) rm kairoschain || true
	# create a new container from the latest image
	$(DOCKER) create --name kairoschain -t -i ${DOCKER_IMAGE}:latest kairoschain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp kairoschain:/usr/bin/kairosd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/HarryBin2002/kairoschain"
	godoc -http=:6060
...
localnet-start: localnet-stop
	@if ! [ -f build/node0/$(APP_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/kairoschain:Z kairoschain/node "./kairosd testnet init-files --v 4 -o /kairoschain --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\kairosd:/kairoschain\Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)\build\node1\kairosd:/kairoschain\Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)\build\node2\kairosd:/kairoschain\Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)\build\node3\kairosd:/kairoschain\Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
else
	@docker run --rm -v $(CURDIR)/build/node0/kairosd:/kairoschain:Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)/build/node1/kairosd:/kairoschain:Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)/build/node2/kairosd:/kairoschain:Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
	@docker run --rm -v $(CURDIR)/build/node3/kairosd:/kairoschain:Z kairoschain/node "./kairosd tendermint unsafe-reset-all --home=/kairoschain"
endif
...
PACKAGE_NAME:=github.com/HarryBin2002/kairoschain
